...
def _load_header(fid, pointer):
    ...
    if pointer != 0 and pointer is not None:
        ...
        (
            ...
            temp["length"],
            temp["link_count"],
        ) = _HeaderStruct.unpack(fid.read(24))
    ...
    def read(self, fid):
        ...
        if self["id_ver"] > 410 and b"UnFin" in self["id_file"]:
            ...
            if self["id_unfi_flags"] & (1 << 4):
                warn(
                    "Update of last DL block in each chained list of "
                    "DL blocks required"
                )
            if self["id_unfi_flags"] & (1 << 5):
                warn(
                    "Update of cg_data_bytes and cg_inval_bytes in VLSD "
                    "CG block required"
                )
...
class HDBlock(dict):
    ...
    def read(self, fid=None):
        ...
        ) = unpack("<4sI9Q2h4B2Q", fid.read(104))
...
class CommentBlock(dict):
    ...
    def read_tx(self, fid, pointer):
        ...
        if pointer > 0:
            ...
            self["Comment"] = (
                fid.read(self["length"] - 24).rstrip(b"\x00").decode("UTF-8", "ignore")
            )
            ...
            if self["cn_attachment_count"] > 0:
                ...
                if self["cn_attachment_count"] > 1:
                    ...
                    for at in range(self["cn_attachment_count"]):
                        self["attachment"][at] = ATBlock(
                            kargs["fid"], self["cn_at_reference"][at]
                        )
                else:
                    self["cn_at_reference"] = links
                    self["attachment"][0] = ATBlock(
                        kargs["fid"], self["cn_at_reference"]
                    )
    ...
    def read(self, fid, pointer):
        ...
        if pointer != 0 and pointer is not None:
            ...
            if 1 << 0 & self["ca_flags"]:  # bit 0
                self["ca_dynamic_size"] = _mdf_block_read(
                    fid, _LINK, self["ca_ndim"] * 3
                )
            if 1 << 1 & self["ca_flags"]:  # bit 1
                self["ca_input_quantity"] = _mdf_block_read(
                    fid, _LINK, self["ca_ndim"] * 3
                )
            ...
            if 1 << 4 & self["ca_flags"]:  # bit 4
                self["ca_cc_axis_conversion"] = _mdf_block_read(
                    fid, _LINK, self["ca_ndim"]
                )
            ...
            if self["ev_attachment_count"] > 0:
                if self["ev_attachment_count"] > 1:
                    self["ev_at_reference"] = links[
                        self["ev_scope_count"] : self["ev_scope_count"]
                        + self["ev_attachment_count"]
                    ]
                else:
                    self["ev_at_reference"] = links
...
class LDBlock(dict):
    ...
    def read_ld(self, fid, link_count):
        ...
        self["list_data"] = {}
        self["list_data"][0] = list(
            unpack("<{}Q".format(link_count - 1), fid.read(8 * (link_count - 1)))
        )
        ...
        if self["flags"]:  # flags existing
            if self["flags"] & (1 << 31):  # invalid data present
                self["inval_data"] = {}
                self["inval_data"][0] = [
                    self["list_data"][0].pop(i)
                    for i in range((link_count - 1) // 2, link_count - 1)
                ]
            if self["flags"] & 0b1:  # equal length data list
                ...
                self["sample_offset"] = unpack(
                    "<{}Q".format(self["count"]), fid.read(8 * self["count"])
                )
            if self["flags"] & 0b10:  # time values
                self["time_values"] = unpack(
                    "<{}Q".format(self["count"]), fid.read(8 * self["count"])
                )
            if self["flags"] & 0b100:  # angle values
                self["angle_values"] = unpack(
                    "<{}Q".format(self["count"]), fid.read(8 * self["count"])
                )
            if self["flags"] & 0b1000:  # distance values
                self["distance_values"] = unpack(
                    "<{}Q".format(self["count"]), fid.read(8 * self["count"])
                )
    ...
    def write(self, fid, data, invalid_data=None, compression_flag=False):
        ...
        if invalid_data is not None:
            fid.write(
                pack("{0}Q".format(number_ld), *zeros(shape=number_ld, dtype="<u8"))
            )
        ...
        if compression_flag:
            if self["column_oriented_flag"]:
                dz_zip_type = (
                    0  # for a column oriented channel, no meaning to have transposition
                )
            else:
...
class DLBlock(dict):
    ...
    def read_dl(self, fid, link_count):
        ...
        self["next"] = unpack("<Q", fid.read(8))[0]
        self["list_data"] = {}
        self["list_data"][0] = unpack(
            "<{}Q".format(link_count - 1), fid.read(8 * (link_count - 1))
        )
...
class HLBlock(dict):
    ...
    def write(self, fid, data):
        ...
        for counter, (n_record_chunk, chunk_size) in enumerate(self["chunks"]):
            ...
            position = DZ.write(
                fid,
                data[data_pointer : data_pointer + chunk_size],
                self["record_length"],
            )
            if position is not None:
                pointer = position
...
class Info4(dict):
    ...
    def read_cg_block(self, fid, dg, channel_name_list=False, minimal=0):
        ...
        if self["DG"][dg]["dg_cg_first"]:
            ...
            if (
                self["CN"][dg][cg]
                and self["CN"][dg][cg][0]["unique_channel_in_CG"]
            ...
            while self["CG"][dg][cg]["cg_cg_next"]:
                ...
                vlsd = False  # flag for at least one VLSD channel in CG
                self["CG"][dg][cg] = {}
                self["CG"][dg][cg].update(
                    CGBlock(fid, self["CG"][dg][cg - 1]["cg_cg_next"])
                )
                self["CN"][dg][cg] = {}
                self["CC"][dg][cg] = {}
                ...
                if (
                    not self["CG"][dg][cg]["cg_flags"] & 0b1
                ):  # if not a VLSD channel group
                    # reads Channel Block
                    vlsd = self.read_cn_block(fid, dg, cg, channel_name_list, minimal)
                    if vlsd:
                        # VLSD needs to rename and append records but with python 2.x impossible,
            ...
            for VLSDcg in vlsd_cg_block:
                ...
                for cg in self["CG"][dg]:
                    if cg not in vlsd_cg_block:
                        for cn in self["CN"][dg][cg]:
                            if (
                                vlsd_cg_block_address
                                == self["CN"][dg][cg][cn]["cn_data"]
                            ):
                                # found matching channel with VLSD CGBlock
                                temp = {}
                                temp["cg_cn"] = (cg, cn)
                                self["VLSD_CG"][
                                    self["CG"][dg][VLSDcg]["cg_record_id"]
                                ] = temp
                                break

            if not self["DG"][dg][
                "unique_channel_in_DG"
            ]:  # no need to order if unique in CG
                ...
                Map = zeros(
                    shape=n_channel, dtype=[("index", "u4"), ("bit_offset", "u4")]
                )
                for cn in range(n_channel):
                    Map[cn] = (
                        cn,
                        self["CN"][dg][cg][cn]["cn_byte_offset"] * 8
                        + self["CN"][dg][cg][cn]["cn_bit_offset"],
                    )
                ...
                for cn in range(n_channel):
                    if not to_change_index[cn]:
                        ...
                        self["CN"][dg][cg][cn] = self["CN"][dg][cg].pop(
                            ordered_map[cn][0] + n_channel
                        )
                        self["CC"][dg][cg][cn] = self["CC"][dg][cg].pop(
                            ordered_map[cn][0] + n_channel
                        )
    ...
    def read_cn_block(self, fid, dg, cg, channel_name_list=False, minimal=0):
        ...
        self["CN"][dg][cg][cn]["name"] = self._unique_channel_name(
            ...
            if not channel_name_list:
                ...
                if self["CN"][dg][cg][cn]["cn_composition"]:
                    ...
                    if id in ("##CA", b"##CA"):
                        ...
                        temp = CNBlock()
                        temp.read_cn(
                            fid=fid, pointer=self["CN"][dg][cg][cn]["cn_composition"]
                        )
        ...
        for MLSDcn in mlsd_channels:
            for cn in self["CN"][dg][cg]:
                if (
                    self["CN"][dg][cg][cn]["pointer"]
                    == self["CN"][dg][cg][MLSDcn]["cn_data"]
                ):
        ...
        for cn in list(self["CN"][dg][cg].keys()):
            if self["CN"][dg][cg][cn]["cn_composition"]:
                ...
                if ID in ("##CN", b"##CN"):  # Structures
                    ...
                    self["CN"][dg][cg][chan].read_cn(
                        fid=fid, pointer=self["CN"][dg][cg][cn]["cn_composition"]
                    )
                    # keep original non unique channel name
                    self["CN"][dg][cg][chan]["orig_name"] = self["CN"][dg][cg][chan][
                        "name"
                    ]
                    # make sure channel name is unique
                    self["CN"][dg][cg][chan]["name"] = self._unique_channel_name(
                        fid, self["CN"][dg][cg][chan]["name"], dg, cg, chan
                    )

                    self["CC"][dg][cg][chan] = CCBlock()
                    self["CC"][dg][cg][chan].read_cc(
                        fid, self["CN"][dg][cg][chan]["cn_cc_conversion"]
                    )
                    ...
                    while self["CN"][dg][cg][chan]["cn_cn_next"]:
                        ...
                        self["CN"][dg][cg][chan] = CNBlock()
                        self["CN"][dg][cg][chan].read_cn(
                            fid=fid, pointer=self["CN"][dg][cg][chan - 1]["cn_cn_next"]
                        )
                        # keep original non unique channel name
                        self["CN"][dg][cg][chan]["orig_name"] = self["CN"][dg][cg][
                            chan
                        ]["name"]
                        # make sure channel name is unique
                        self["CN"][dg][cg][chan]["name"] = self._unique_channel_name(
    ...
    def _unique_channel_name(self, fid, name, dg, cg, cn):
        ...
        if self["CG"][dg][cg]["link_count"] > 6:  # cg master link
            try:
                self["masters"][self["CG"][dg][cg]["cg_cg_master"]]["channels"].add(
                    name
                )
            except KeyError:
                ...
                if info["CN"][dg][cg][cn]["cn_data_type"] == 13:
                    ...
                    if channel_list is None or name in channel_list:
                        ...
                        if "description" in info["CN"][dg][cg][cn]:
                            mdfdict[name][descriptionField] = info["CN"][dg][cg][cn][
                                "description"
                            ]
                        else:
