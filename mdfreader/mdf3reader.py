...
def _linear_conversion(
    data, conversion
):  # 0 Parametric, Linear: Physical =Integer*P2 + P1
...
def _polynomial_conversion(data, conversion):  # 6 Polynomial
    ...
    return (
        conversion["P2"]
        - conversion["P4"] * (data - conversion["P5"] - conversion["P6"])
    ) / (
        conversion["P3"] * (data - conversion["P5"] - conversion["P6"])
        - conversion["P1"]
    )
...
def _exponential_conversion(data, conversion):  # 7 Exponential
    ...
    if conversion["P4"] == 0 and conversion["P1"] != 0 and conversion["P2"] != 0:
        return (
            exp(
                ((data - conversion["P7"]) * conversion["P6"] - conversion["P3"])
                / conversion["P1"]
            )
            / conversion["P2"]
        )
        ...
        return (
            exp(
                (conversion["P3"] / (data - conversion["P7"]) - conversion["P6"])
                / conversion["P4"]
            )
            / conversion["P5"]
        )
...
def _rational_conversion(data, conversion):  # 9 rational
    ...
    """
    return (
        conversion["P1"] * data * data + conversion["P2"] * data + conversion["P3"]
    ...
    try:
        ...
        for pair in range(n_pair):
            ...
            if "LINEAR_CONV" in text[pair]:  # linear conversion from CANape
                ...
                text[pair] = (
                    text[pair][left + 1 : right].replace("{", "").replace("}", "")
                )
...
class Record(list):
    ...
    def load_info(self, info):
        ...
        self.CGrecordLength = info["CGBlock"][self.dataGroup][self.channelGroup][
            "dataRecordSize"
        ]
        self.numberOfRecords = info["CGBlock"][self.dataGroup][self.channelGroup][
            "numberOfRecords"
        ]
        ...
        for channelNumber in range(
            info["CGBlock"][self.dataGroup][self.channelGroup]["numberOfChannels"]
        ):
            channel = Channel3(
                info,
                self.dataGroup,
            ...
            if (
                self.master["number"] is None or channel.channelType == 1
            ):  # master channel found
            ...
            if len(self) > 1:
                ...
                prev_chan_includes_curr_chan = (
                    ...
                    and channel.posBitEnd
                    <= 8 * (prev_chan.byteOffset + prev_chan.nBytes_aligned)
                )
                ...
                if (
                    ...
                    and channel.posBitEnd > embedding_channel.posByteEnd * 8
                ):
                    embedding_channel = None
                if (
                    prev_chan_includes_curr_chan or embedding_channel_includes_curr_chan
                ):  # bit(s) in byte(s)
                ...
                try:  # use rather cython compiled code for performance
                    ...
                    for r in range(self.numberOfRecords):  # for each record,
                        ...
                        for channel in rec_chan:
                            rec[channel.name][r] = channel.CFormat.unpack(
        ...
        for Channel in self:  # list of channel classes from channelSet
            if Channel.name in channel_set:
                ...
                if not n_bytes == Channel.nBytes_aligned:
                    delta_byte = bitarray(
                        8 * (Channel.nBytes_aligned - n_bytes), endian="little"
                    )
                ...
                if (
                    Channel.signalDataType in (1, 10, 14)
                    and n_bytes == Channel.nBytes_aligned
                    and n_trail_bits > 0
                ...
                if "s" not in Channel.dataFormat:
                    temp[Channel.name] = Channel.CFormat.unpack(
                        temp[Channel.name].tobytes()
                    )[0]
                else:
...
class DATA(dict):
    ...
    def read(self, channel_set, file_name):
        ...
        if len(self) == 1:  # sorted dataGroup
            ...
            self[record_id]["data"] = self.load_sorted(
                self[record_id]["record"], name_list=channel_set
            )
    ...
    def load_unsorted(self, name_list=None):
        ...
        for record_id in self:
            for channelName in self[record_id]["record"].dataRecordName:
                buf[channelName] = []
            if (
                self[record_id]["record"].hiddenBytes
                or not self[record_id]["record"].byte_aligned
            ):
        ...
        while position < len(stream):
            ...
            if (
                not self[record_id]["record"].hiddenBytes
                and self[record_id]["record"].byte_aligned
            ):
...
class Mdf3(MdfSkeleton):
    ...
    def read3(
        ...
        if info is None:
            if self.info is None:
                info = Info3(
                    self.fileName, fid=None, filter_channel_names=False, minimal=minimal
                )
            else:
                info = self.info
            ...
            if info["DGBlock"][dataGroup]["numberOfChannelGroups"] > 0 and (
                ...
                if "dataClass" not in info["DGBlock"][dataGroup]:
                    ...
                    for channelGroup in range(
                        info["DGBlock"][dataGroup]["numberOfChannelGroups"]
                    ):
                        ...
                        ):  # continue if there are at least some records
                            buf.add_record(temp)
                ...
                buf.read(
                    channel_set, self.fileName
                )  # reads datablock potentially containing several channel groups

                channel_groups = buf
                if self._noDataLoading and channel_list is not None:
                    channel_groups = [
                        info["CGBlock"][dataGroup][self[channel][idField][1]][
                            "recordID"
                        ]
                                ...
                                if chan.bit_masking_needed:
                                    ...
                                    if chan.signalDataType in (
                                        ...
                                        if chan.embedding_channel_bitOffset > 0:
                                            temp = right_shift(
                                                temp, chan.embedding_channel_bitOffset
                                            )
                                        mask = int(
                                            pow(2, chan.bitCount) - 1
                                        )  # masks isBitUint8
                                        temp = bitwise_and(temp, mask)
                                    else:  # should not happen
                                        warn(
                                            "bit count and offset not applied to correct data type"
                                        )
                                self.add_channel(
                                    chan.name,
                                    temp,
        ...
        if channel_name in self:
            ...
            if vector is None:  # noDataLoading reading argument flag activated
                if self.info.fid is None or (
                    ...
                    (self.info.fid, self.info.fileName, zipfile) = _open_mdf(
                        self.fileName
                    )
                self.read3(
                    file_name=None,
    ...
    def _convert3(self, channel_name, convert_tables=False):
        ...
        if self[channel_name][dataField] is None:
            ...
            if isinstance(self[channel_name][dataField], CompressedData):
                vector = self[channel_name][
                    dataField
                ].decompression()  # uncompress blosc
            else:
                vector = self[channel_name][dataField][
                    :
    ...
    def _convert_channel3(self, channel_name):
        ...
        self.set_channel_data(
            channel_name, self._convert3(channel_name, self.convertTables)
        )
    ...
    def write3(self, file_name=None):
        ...
        if self.fileMetadata["author"] is not None:  # Author
            try:
                author = "{:\x00<32.31}".format(self.fileMetadata["author"]).encode(
                    "latin-1", "ignore"
                )
        ...
        if self.fileMetadata["project"] is not None:  # Project
            try:
                project = "{:\x00<32.31}".format(self.fileMetadata["project"]).encode(
                    "latin-1", "ignore"
                )
            except UnicodeEncodeError:
                project = b"\x00" * 32
        ...
        if self.fileMetadata["subject"] is not None:  # Subject
            try:
                subject = "{:\x00<32.31}".format(self.fileMetadata["subject"]).encode(
                    "latin-1", "ignore"
                )
            except UnicodeEncodeError:
                subject = b"\x00" * 32
        ...
        for masterChannel in self.masterChannelList:
            ...
            for channel in self.masterChannelList[masterChannel]:
                ...
                if data.dtype.kind not in ["S", "U"]:
                    ...
                    data_type_list = "".join(
                        [data_type_list, "{}s".format(data.dtype.itemsize)]
                    )
