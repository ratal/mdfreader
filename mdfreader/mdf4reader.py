...
chunk_size_reading = (
    100000000  # reads by chunk of 100Mb, can be tuned for best performance
)
    ...
    if parent_block["id"] in (
        ...
        if sorted_flag:
            if (
                ...
                if info["DG"][record.dataGroup][
                    "unique_channel_in_DG"
                ] and parent_block["id"] in (b"##DV", "##DV"):
                    return frombuffer(
                        parent_block["data"],
                        dtype={
                            "names": record.dataRecordName,
                ...
                return record.read_channels_from_bytes(
                    parent_block["data"], info, channel_set, n_records
                )
        ...
        if (
            channel_set is None and sorted_flag
        ):  # reads all blocks if sorted block and no channelSet defined
            ...
            channel_set is not None and sorted_flag
        ):  # sorted data but channel list requested
            return record.read_channels_from_bytes(
                parent_block["data"], info, channel_set, n_records
            )
        else:  # unsorted reading
        ...
        return frombuffer(
            parent_block["data"],
            dtype={
                "names": [record.invalid_channel.name],
...
def _read_unsorted(record, info, parent_block, channel_set=None):
    ...
    while position < len(parent_block["data"]):
        ...
        if not record[record_id]["record"].Flags & 0b1:  # not VLSD CG)
            temp = record.read_record(
                record_id,
                info,
                parent_block["data"][
                    position : position + record[record_id]["record"].CGrecordLength + 1
                ],
            )
            ...
            VLSDLen = VLSDStruct.unpack(parent_block["data"][position : position + 4])[
                0
            ]  # VLSD length
            position += 4
            ...
            signal_data_type = (
                ...
                .VLSD_CG[record_id]["channel"]
                .signal_data_type(info)
            )
            if signal_data_type == 6:
                ...
                temp = temp.decode(">utf-16")
            buf[record[record_id]["record"].VLSD_CG[record_id]["channelName"]].append(
                temp
            )
            position += VLSDLen
...
def _read_sd_block(signal_data_type, sd_block, sd_block_length):
    ...
    if signal_data_type < 10:
        ...
        while pointer < sd_block_length:
            VLSDLen = structunpack("I", sd_block[pointer : pointer + 4])[
                0
            ]  # length of data
            pointer += 4
            buf.append(
                sd_block[pointer : pointer + VLSDLen]
                .decode(channel_format)
                .rstrip("\x00")
            )
            pointer += VLSDLen
...
class Data(dict):
    ...
    def add_record(self, record):
        ...
        """
        if (
            record.recordID not in self
        ):  # do not overwrite already existing record in Data, for VLSD_CG
            ...
            for recordID in self[record.recordID]["record"].VLSD_CG:
                ...
                self[recordID]["record"].VLSD_CG = self[record.recordID][
                    "record"
                ].VLSD_CG
        else:  # VLSD_CG
            ...
            for cn in record.VLSD:  # VLSD channels
                if channel_set is None or record[cn].name in channel_set:
                    ...
                    temp, invalid = temp.load(
                        record, info, name_list=channel_set, sorted_flag=True, vlsd=True
                    )
                    self[recordID]["data"] = rename_fields(
                        self[recordID]["data"],
                        {record[cn].name: "{}_offset".format(record[cn].name)},
                    )
                    self[recordID]["data"] = rec_append_fields(
                        self[recordID]["data"], record[cn].name, temp
                    )
    ...
    def read_data_list(
        self, field, nBytes, temps, record, info, name_list, sorted_flag, vlsd
    ):
        previous_index = 0
        data_block = defaultdict()
        data_block["data"] = bytearray()
...
class Record(list):
    ...
    def add_channel(self, info, channel_number):
        ...
        channel = Channel4()
        self.append(
            channel.set(info, self.dataGroup, self.channelGroup, channel_number)
        )
    ...
    def load_info(self, info):
        ...
        info : mdfinfo4.info4 class
        """
        self.CGrecordLength = info["CG"][self.dataGroup][self.channelGroup][
            "cg_data_bytes"
        ]
        ...
        self.numberOfRecords = info["CG"][self.dataGroup][self.channelGroup][
            "cg_cycle_count"
        ]
        ...
        for channelNumber in info["CN"][self.dataGroup][self.channelGroup]:
            ...
            self.master = info["masters"][
                info["CN"][self.dataGroup][self.channelGroup][channelNumber]["masterCG"]
            ]["name"]
            if channel_type in (0, 1, 2, 4, 5):  # not virtual channel
                ...
                if signal_data_type == 13:
                    for name in ("ms", "minute", "hour", "day", "month", "year"):
                        ...
                        channel.set_CANOpen(
                            info, self.dataGroup, self.channelGroup, channelNumber, name
                        )
                        self.append(channel)
                    ...
                    if len(self) > 1:
                        ...
                        if (
                            prev_chan_includes_curr_chan
                            or embedding_channel_includes_curr_chan
                        ):  # bit(s) in byte(s)
                            if (
                                embedding_channel is None
                                and prev_chan_includes_curr_chan
                            ):
                                embedding_channel = (
                                    prev_chan  # new embedding channel detected
                                )
                    ...
                    if "VLSD_CG" in info:  # is there VLSD CG
                        for recordID in info["VLSD_CG"]:  # look for VLSD CG Channel
                            if info["VLSD_CG"][recordID]["cg_cn"] == (
                                self.channelGroup,
                                channelNumber,
                            ):
        ...
        if info["CG"][self.dataGroup][self.channelGroup][
            "cg_invalid_bytes"
        ]:  # invalid bytes existing
            invalid_bytes = Channel4()
            invalid_bytes.set_invalid_bytes(
                info, self.dataGroup, self.channelGroup, channelNumber + 1
            )
            ...
            if info["DG"][self.dataGroup]["data_block_header"]["id"] not in (
                b"##LD",
                "##LD",
            ):
                ...
                self.recordLength += info["CG"][self.dataGroup][self.channelGroup][
                    "cg_invalid_bytes"
                ]
        ...
        # check record length consistency
            ...
            self.byte_aligned = (
                False  # forces to use dataRead instead of numpy records.
            )
        ...
        if channel_set is None and self.byte_aligned and not self.hiddenBytes:
            ...
            if channel_set is None or len(channel_set & self.channelNames) > 0:
                if self.unique_channel_in_DG:
                    ...
                    return self.read_not_all_channels_sorted_record(
                        fid, info, channel_set
                    )
    ...
    def generate_chunks(self):
        ...
        """
        n_chunks = (
            self.CGrecordLength * self.numberOfRecords
        ) // chunk_size_reading + 1
    ...
    def read_not_all_channels_sorted_record(self, fid, info, channel_set):
        ...
        if rec is not None:
            if dataRead_available:
                for n_record_chunk, chunk_size in chunks:
                    rec[
                        previous_index : previous_index + n_record_chunk
                    ] = self.read_channels_from_bytes(
                        fid.read(chunk_size),
                        info,
                        channel_set,
                        n_record_chunk,
                ...
                for n_record_chunk, chunk_size in chunks:
                    rec[
                        previous_index : previous_index + n_record_chunk
                    ] = self.read_channels_from_bytes_fallback(
                        fid.read(chunk_size),
                        info,
                        channel_set,
                        n_record_chunk,
    ...
    def initialise_recarray(
        self, info, channel_set, n_records, dtype=None, channels_indexes=None
    ):
        """ Initialise recarray
        ...
        if dtype is not None and channels_indexes is not None:
            ...
            for chan in range(len(self)):
                if self[chan].name in channel_set and self[chan].channel_type(
                    info
                ) not in (3, 6):
    ...
    def read_channels_from_bytes(
        ...
        if buf is not None:  # at least some channels should be parsed
            if dataRead_available:  # use rather cython compiled code for performance
                ...
                return self.read_channels_from_bytes_fallback(
                    bit_stream, info, channel_set, n_records, dtype
                )
        else:
            return []
        ...
        if buf is not None:
            ...
            for chan in channels_indexes:
                ...
                if not self[chan].type in (1, 2):
                    ...
                    if (
                        signal_data_type in (2, 3)
                        and n_bytes == n_bytes_estimated
                        and n_trail_bits > 0
                    ...
                    temp = [
                        bit_array[
                            self[chan].pos_bit_beg(info)
                            + record_bit_size * i : self[chan].pos_bit_beg(info)
                            + 8 * n_bytes_estimated
                            + record_bit_size * i
                        ]
                        for i in range(n_records)
                ...
                if "s" not in self[chan].c_format(info):
                    ...
                    if (
                        ">" in self[chan].data_format(info) and byteorder == "little"
                    ) or (byteorder == "big" and "<" in self[chan].data_format(info)):
                        temp = [
                            c_structure.unpack(temp[i].tobytes())[0]
                            for i in range(n_records)
                        ]
...
class Mdf4(MdfSkeleton):
    ...
    def read4(
        ...
        for dataGroup in data_groups:
            ...
            if not info["DG"][dataGroup]["dg_data"] == 0 and (
                channel_set is None
                or len(channel_set & info["ChannelNamesByDG"][dataGroup]) > 0
            ):  # there is data block and channel in
                if (
                    minimal > 1 and not self._noDataLoading
                ):  # load CG, CN and CC block info
                    info.read_cg_block(
                        info.fid, dataGroup, channel_set, minimal=minimal
                    )
                if info["CG"][dataGroup][0]["cg_cycle_count"]:  # data exists
                    ...
                    if "dataClass" not in info["DG"][dataGroup]:
                        ...
                        for channelGroup in info["CG"][dataGroup]:
                            ...
                            record_id = info["CG"][dataGroup][channelGroup][
                                "cg_record_id"
                            ]
                            if (
                                temp.master is not None
                            ...
                            if (
                                channel_set is not None
                                and buf[record_id]["record"].CANOpen
                            ):
                    ...
                    if self._noDataLoading and channel_list is not None:
                        channel_groups = [
                            info["CG"][dataGroup][self[channel][idField][0][1]][
                                "cg_record_id"
                            ]
                    ...
                    for (
                        record_id
                    ) in channel_groups:  # for each channel group in data block
                        if "record" in buf[record_id]:
                            ...
                            if self._noDataLoading and channel_list is not None:
                                channels = [
                                    channels[self[channel][idField][0][2]]
                                    for channel in channel_list
                                ]
                            for chan in channels:  # for each channel class
                                if channel_set is None or chan.name in channel_set:
                                    if not chan.type == 4:  # normal channel
                                        if chan.channel_type(info) not in (
                                            3,
                                            6,
                                        ):  # not virtual channel
                                            ...
                                            if (
                                                ...
                                                record_name = buf[record_id][
                                                    "record"
                                                ].recordToChannelMatching[chan.name]
                                            else:
                                                record_name = chan.name
                                            if (
                                                "data" in buf[record_id]
                                                and buf[record_id]["data"] is not None
                                            ):  # no data in channel group
                                                temp = buf[record_id]["data"][
                                                    record_name
                                        ...
                                        if temp is not None:  # channel contains data
                                            ...
                                            if (
                                                ...
                                                self.set_invalid_channel(
                                                    chan.name,
                                                    "invalid_bytes{}".format(dataGroup),
                                                )
                                    else:  # invalid bytes channel
                                        if buf[record_id]["invalid_data"] is None:
                                            invalid_data = buf[record_id][
                                                "data"
                                            ].__getattribute__(chan.name)
                                        else:
                                            invalid_data = buf[record_id][
                                                "invalid_data"
                                            ]
                                        if not info["DG"][dataGroup][
                                            "unique_channel_in_DG"
                                        ]:
                                            invalid_data = frombuffer(
                                                invalid_data.tobytes(), dtype="u1"
                                            ).reshape(
                                                len(invalid_data),
    ...
    def _get_channel_data4(self, channel_name, raw_data=False):
        ...
        if channel_name in self:
            ...
            if vector is None:  # noDataLoading reading argument flag activated
                if self.info.fid is None or (
                    self.info.fid is not None and self.info.fid.closed
                ):
                    (self.info.fid, self.info.fileName, self.info.zipfile) = _open_mdf(
                        self.fileName
                    )
                self.read4(
                    file_name=None,
            ...
            if not raw_data:
                return self._convert_channel_data4(
                    self.get_channel(channel_name), channel_name, self.convertTables
                )[channel_name]
            else:
    ...
    @staticmethod
    def _convert_channel_data4(
        channel, channel_name, convert_tables, multi_processed=False, q=None
    ):
        """converts specific channel from raw to physical data according to CCBlock information
        ...
        if (
            conversionField in channel and channel[conversionField]["type"]
        ):  # there is conversion property
            text_type = vector.dtype.kind in [
                "S",
                "U",
                "V",
            ]  # channel of string or not ?
                ...
                for channelName in self:
                    ...
                    if "conversion" in channel:
                        ...
                        if conversion["type"] in (1, 2):  # more time in multi proc
                            ...
                            proc.append(
                                Process(
                                    target=self._convert_channel_data4,
                                    args=(
                                        channel,
                                        channelName,
                                        self.convertTables,
    ...
    def write4(self, file_name=None, compression=False, column_oriented=False):
        ...
        pointer = (
            blocks["HD_comment"]["block_start"] + blocks["HD_comment"]["block_length"]
        )
        ...
        pointer = (
            blocks["FH_comment"]["block_start"] + blocks["FH_comment"]["block_length"]
        )
            ...
            if column_oriented:
                for dataGroup, masterChannel in enumerate(self.masterChannelList):
                    ...
                    if master_channel_data is not None:
                        ...
                        master_channel_data not in self.masterChannelList[masterChannel]
                    ):
                        cg_cycle_count = len(
                            self._get_channel_data4(
                                self.masterChannelList[masterChannel][0]
                        ...
                        warn(
                            "no data in data group {0} with master channel {1}".format(
    ...
    def _write4_non_column(self, fid, pointer, compression=False):
        ...
        for dataGroup, masterChannel in enumerate(self.masterChannelList):
            ...
            if master_channel_data is not None:
                ...
                self._get_channel_data4(self.masterChannelList[masterChannel][0]).shape[
                    0
                ]
                == 1
            ):  # classification
                ...
                cg_cycle_count = len(
                    self._get_channel_data4(self.masterChannelList[masterChannel][0])
                )
                warn("no master channel in data group {}".format(dataGroup))
            else:
                # no data in data group, skip
            ...
            for n_channel, channel in enumerate(self.masterChannelList[masterChannel]):
                ...
                if (
                    ...
                    if issubdtype(data.dtype, numpy_number):  # is numeric
                        ...
                        blocks[n_channel][
                            "cn_flags"
                        ] = 8  # only Bit 3: Limit range valid flag
                    else:
                    ...
                    if cn_flag:
                        ...
                        blocks[n_channel][
                            "CN"
                        ] = 0  # creates first CN link, null for the moment
                    ...
                    pointer = (
                        blocks[channel]["block_start"] + blocks[channel]["block_length"]
                    )
            ...
            if (
                n_records == 0
                and masterChannel is not self.masterChannelList[masterChannel]
            ):
    ...
    def _write4_column(
        ...
        if self.get_invalid_channel(channel) or isinstance(data, MaskedArray):
            ...
            blocks["CG"][
                "cg_inval_bytes"
            ] = 1  # as column oriented, only one byte for DIBlock
        else:
        ...
        if data is not None and len(data) > 0:
            ...
            if (
                issubdtype(data.dtype, numpy_number) and not invalid_channel
            ):  # is numeric
            ...
            if unit is not None and len(unit) > 0:
                ...
                blocks[unit_name].load(unit, "TX")
                pointer = (
                    blocks[unit_name]["block_start"] + blocks[unit_name]["block_length"]
                )
            else:
                blocks["CN"]["Unit"] = 0
            ...
            if desc is not None and len(desc) > 0:
                ...
                blocks[desc_name].load(desc, "TX")
                pointer = (
                    blocks[desc_name]["block_start"] + blocks[desc_name]["block_length"]
                )
            else:
                blocks["CN"]["Comment"] = 0
            ...
            if not invalid_channel and not compression:
                ...
                pointer = data_blocks.write(
                    fid,
                    data.tobytes(),
                    invalid_data=inval_data,
...
def _rational_conversion(vector, cc_val):
    ...
    p5 = cc_val[4]
    p6 = cc_val[5]
    return (p1 * vector * vector + p2 * vector + p3) / (
        p4 * vector * vector + p5 * vector + p6
    )
...
def _value_to_text_conversion(vector, cc_val, cc_ref):
    ...
    temp = empty(
        len(vector), dtype="U{}".format(maxlen)
    ...
    for ref in range(len(cc_ref)):
        if isinstance(cc_ref[ref], CCBlock):
            if cc_ref[ref]["cc_type"] == 3:
                ...
                cc_ref[ref] = lambdify(
                    X, cc_ref[ref]["cc_ref"]["Comment"], modules="numpy", dummify=False
                )
            elif cc_ref[ref]["cc_type"] == 1:  # linear conversion
                cc_ref[ref] = lambdify(
                    X,
                    "{0}* X + {1}".format(
                        cc_ref[ref]["cc_val"][1], cc_ref[ref]["cc_val"][0]
                    ),
                    modules="numpy",
                    dummify=False,
                )
            ...
            PythonVersion < 3 and not isinstance(cc_ref[ref], unicode)
        ):  # identity, non conversion
    ...
    for ref in range(len(cc_ref)):
        if isinstance(cc_ref[ref], CCBlock):
            if cc_ref[ref]["cc_type"] == 3:
                ...
                cc_ref[ref] = lambdify(
                    X,
                    "{0} * X + {1}".format(cc_val[1], cc_val[0]),
                    modules="numpy",
                    dummify=False,
                )
        ...
        for i in range(ref_count):
            ...
            if cc_ref[i]:  # not NIL link
                if cc_ref[i]["cc_type"] == 7:
                    assembled_string += _value_to_text_conversion(
