...
class MainWindow(QMainWindow, Ui_MainWindow, QFileDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.actionPlotSelectedChannel = QAction(
            "Plot", self.SelectedChannelList
        )  # context menu to allow plot of channel
        self.SelectedChannelList.addAction(self.actionPlotSelectedChannel)
        self.connect(
            self.actionPlotSelectedChannel, SIGNAL("triggered()"), self.plotSelected
        )
        self.actionPlotChannel = QAction(
            "Plot", self.channelList
        )  # context menu to allow plot of channel
        ...
        )  # context menu to remove selected file from list
    ...
    def on_browse_clicked(self):
        ...
        if self.defaultPath is None:
            self.fileNames = QFileDialog.getOpenFileNames(
                ...
                filter=("MDF file (*.dat *.mdf *.mf4 *.mfx *.mfxz)"),
            )
        ...
        if not len(self.fileNames) == 0:
            ...
            ChannelList = convertChannelList(
                self.mdfinfoClass.list_channels(str(self.fileNames[0]))
            )
    ...
    def cleanSelectedChannelList(self):
        ...
        [
            self.SelectedChannelList.takeItem(0)
            for i in range(self.SelectedChannelList.count())
        ]
    ...
    def on_Convert_clicked(self):
       ...
       Will convert mdf files into selected format
        ...
        channelList = []
        [
            channelList.append(str(self.SelectedChannelList.item(i).text()))
            for i in range(self.SelectedChannelList.count())
        ]
        ...
        if self.FileList.count() > 0:  # not empty list
            ...
            if (
                self.MergeFiles or self.FileList.count() < 2
            ):  # export all files separately, inverted bool
                convertFlag = True
                ...
                args = [
                    (
                        str(self.FileList.takeItem(0).text()),
                        channelList,
                        resampleFlag,
                        resampleValue,
                        convertFlag,
                ...
                fileName = str(
                    self.FileList.item(0).text()
                )  # Uses first file name for the converted file
                # list filenames
                args = [
                    (
                        str(self.FileList.takeItem(0).text()),
                ...
                self.mdfClass.fileName = (
                    fileName  # First filename will be used for exported file name
                )
                ...
                for res in result:  # Merge
                    ...
                    self.mdfClass.merge_mdf(
                        buffer
                    )  # merge buffer to merged class mdfClass
                # Export
    ...
    def on_FileList_itemClicked(self, item):
        ...
        self.cleanChannelList()
        ChannelList = convertChannelList(
            self.mdfinfoClass.list_channels(str(item.text()))
        )
    ...
    def on_LabFileBrowse_clicked(self):
        ...
        """
        self.labFileName = QFileDialog.getOpenFileName(
            self, "Select Lab Files", filter=("Lab file (*.lab)")
        )
        if not len(self.labFileName) == 0:
            self.LabFile.del_()  # clear linedit
            self.LabFile.insert(
                str(self.labFileName)
            )  # replace linedit field by browsed file name
            # read lab file
    ...
    def FileRemove(self):
        ...
        [
            self.FileList.takeItem(self.FileList.row(selectionList[i]))
...
def processMDF(
    fileName, channelist, resampleFlag, resampleValue, convertFlag, convertSelection
):
...
def processMDFstar(args):
    try:
        ...
        print(
            "Error, following file might be corrupted : " + args[0]
        )  # Shows fileName and parameters to help finding corrupted files
        raise Exception(
            "Please re-try by removing this file from the list and restart mdfconverter to kill processes and clean memory"
        )
